...
static void enc_rotation_handler(void *_) {
  // read GPIOs
  int p1 = gpio_get_level(GPIO_NUM_33);
  int p2 = gpio_get_level(GPIO_NUM_34);
...
void enc_init(enc_callback_t cb) {
  ...
  gpio_config_t rc;
  rc.pin_bit_mask = GPIO_SEL_33 | GPIO_SEL_34;
  ...
  // preset state
  if (gpio_get_level(GPIO_NUM_33)) enc_state |= 1;
  if (gpio_get_level(GPIO_NUM_34)) enc_state |= 2;

  // add interrupt handlers
  gpio_isr_handler_add(GPIO_NUM_33, enc_rotation_handler, NULL);
  gpio_isr_handler_add(GPIO_NUM_34, enc_rotation_handler, NULL);
